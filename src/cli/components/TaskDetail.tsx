...
import { formatChildTasksForDisplay } from '../utils/taskFormatters';
...
const TaskDetail: React.FC<TaskDetailProps> = ({ task, childTasks = [] }) => {
  const [width] = useStdoutDimensions();

  // Prepare child tasks for display if there are any
  const displayChildTasks = childTasks.length > 0
    ? formatChildTasksForDisplay(childTasks)
    : [];
  ...
  return (
    <Box flexDirection="column" padding={1} borderStyle="round" borderColor="blue">
      ...
      {/* Child Tasks Table */}
      {displayChildTasks.length > 0 && (
        <>
          <Box marginY={1}>
            <Divider title="Child Tasks" />
          </Box>
          <TaskList tasks={displayChildTasks} />
        </>
      )}
    </Box>
  );
