...
export async function addTaskLogic(
  ...
  const validatedPriority = TaskPrioritySchema.parse(
    options.priority ?? TaskPrioritySchema.enum.medium
  );
  const validatedType = TaskTypeSchema.parse(
    options.type ?? TaskTypeSchema.enum.feature
  );
  const validatedStatus = TaskStatusSchema.parse(
    options.status ?? TaskStatusSchema.enum.pending
  );
    ...
    .action(async (options: AddTaskOptions) => {
      try {
        ...
        if (options.interactive) {
          ...
          const answers = await inquirer.prompt([
            {
              ...
              validate: (input: string) =>
                input.trim() !== '' || 'Title cannot be empty.',
            },
            {
              type: 'editor', // Or 'input'
              name: 'description',
              message:
                'Description (press Enter to launch editor, ESC to skip):',
            },
            {
              type: 'list',
              name: 'priority',
          ...
          if (!options.title) {
            throw new Error(
              'Task title is required when not using interactive mode.'
            );
          }
        ...
        console.log(
          `✅ Task "${newTask.title}" (ID: ${newTask.id}) created successfully.`
        );
      } catch (error: unknown) {
        const errorMessage =
